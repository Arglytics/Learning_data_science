...
from Projects.Agency.Dormancy.monthly_agency_dormancy import *

agency_month_results=agency_monthly_results()
agency_month_results3=agency_month_results.copy()

agency_month_results3['RM']=agency_month_results3['RM'].fillna('HEADOFFICE')
agency_month_results3=agency_month_results3[~agency_month_results3['Branch'].isna()]
agency_month_results3=agency_month_results3.rename(columns={'BRANCH':'Branch','BRANCH_ID':'Branch_id'})
...
def Branch_email():
    for Branch in agency_month_results3['Branch'].unique():
        ...
        if pd.isnull(names):
            ...
            Please find attached the monthly Branch report on dormant agents.
...
def reg_manager_email():
    for rm in agency_month_results3['RM'].unique():
        ...
        if pd.isnull(names):
            ...
            Please find attached the regional monthly report on agents dormancy.
...
def head_office_email():
    summary1 = agency_month_results3.groupby('RM').agg({'Branch':'nunique','Agent_Acc_Number': 'nunique',
                                              'Terminal_Id': 'nunique'
                                              }).reset_index()
    ...
    ###Branch
    summary1 = agency_month_results3.groupby('Branch').agg({'Agent_Acc_Number': 'nunique',
                                              'Terminal_Id': 'nunique'
                                              }).reset_index()
    summary1.columns = ['Branch', 'No of agents', 'No of Terminals']
    ...
    ##trans)info
    ho_colseq = agency_month_results3.columns[1:2].tolist() + agency_month_results3.columns[17:18].tolist()+ agency_month_results3.columns[2:6].tolist()\
        +agency_month_results3.columns[6:7].tolist() + agency_month_results3.columns[13:17].tolist() + agency_month_results3.columns[18:19].tolist()\
            +agency_month_results3.columns[23:24].tolist()
    ...
    names = ['Gabriel']
    print(ccemails)
    subject = 'Summarized Agent Dormancy monthly Report'
    ...
    Please find attached the summarized monthly report on agents dormancy for all Regions and Branches.
