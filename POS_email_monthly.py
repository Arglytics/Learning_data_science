...
from Projects.Agency.Dormancy.POS_Agents_monthly import *

agency_month_results = pos_agency_monthly_results()
agency_month_results3 = agency_month_results.copy()

agency_month_results3['RM'] = agency_month_results3['RM'].fillna('HEADOFFICE')
agency_month_results3 = agency_month_results3.replace({np.nan: None})
agency_month_results3 = agency_month_results3.replace({0: None})
agency_month_results3['BRANCH'] = agency_month_results3['BRANCH'].str.replace(
    '/', '_')
agency_month_results3 = agency_month_results3.loc[agency_month_results3['BRANCH'] != 'daadab']

Branches = []


def Branch_email():
    for Branch in agency_month_results3['BRANCH'].unique():
        Branch_data = agency_month_results3[agency_month_results3['BRANCH'] == Branch].copy(
        )
        ...
        if names == []:
            ...
            subject = 'monthly_Branch_report'

            message_template = Template('{:<10}'.format(
                """
            Hello ${PERSON_NAME},

            Please find attached the monthly Branch report on dormant POS agents.
...
def reg_manager_email():
    for rm in agency_month_results3['RM'].unique():
        rm_data = agency_month_results3[agency_month_results3['RM'] == rm].copy()
        reg = agency_month_results3['RM'].unique()
        regions.append(reg)
        ...
        ccemails = []
        names = names
        subject = 'Regional monthly Report_'+rm
        ...
        if names == []:
            ...
            subject = 'Regional_monthly_report'

            message_template = Template('{:<10}'.format(
                """
            Hello ${PERSON_NAME},

            Please find attached the regional monthly report on POS agents dormancy.
...
def head_office_email():
    summary1 = agency_month_results3.groupby('RM').agg({'BRANCH': 'nunique', 'TRANSACTING_ACC': 'nunique',
    ...
    # Branch
    summary1 = agency_month_results3.groupby('BRANCH').agg({'TRANSACTING_ACC': 'nunique',
                                                           'OUTLET_CODE': 'nunique', 'TERMINAL_ID': 'nunique'
                                                           }).reset_index()
    summary1.columns = ['Branch', 'No of agents',
                        'No of outlets', 'No of Terminals']
    ...
    # trans)info
    ho_colseq = agency_month_results3.columns[1:2].tolist() + agency_month_results3.columns[18:19].tolist() + agency_month_results3.columns[2:8].tolist()\
        + agency_month_results3.columns[14:18].tolist() + agency_month_results3.columns[19:20].tolist()\
        + agency_month_results3.columns[24:25].tolist()
    trans_info = agency_month_results3[ho_colseq]
    ...
    names = ['Gabriel']
    print(ccemails)
    subject = 'Summarized POS Agents Dormancy monthly Report'
    ...
    Please find attached the summarized monthly report on POS agents dormancy for all regions and branches.
